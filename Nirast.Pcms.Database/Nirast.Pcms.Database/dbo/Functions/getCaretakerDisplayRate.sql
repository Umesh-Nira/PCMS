...
CREATE FUNCTION [dbo].[getCaretakerDisplayRate] (@CareTakerId int, @ServiceId int, @Date datetime)
RETURNS float
AS
BEGIN
...
DECLARE @MaxDate as datetime;
		...
		SELECT @Rate = ISNULL(r.DisplayRate,0) from [Caretaker_Booking_Rates] r
		INNER JOIN [dbo].[Caretaker_Booking_Payrise] p on p.PayriseId = r.BookingPayRiseId
		where r.CaretakerId = @CaretakerId 
			and r.ServiceId = @ServiceId
