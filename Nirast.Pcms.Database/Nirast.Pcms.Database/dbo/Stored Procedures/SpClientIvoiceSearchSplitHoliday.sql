CREATE PROCEDURE [dbo].[SpClientIvoiceSearchSplitHoliday]--null,null,'01-jan-2019','15-jan-2019',null,null,null,4
(
	@CareTakerId int, 
	@CategoryId int, 
	@FromDate datetime,
...
WHERE
    DATEDIFF(dd,cs.StartDateTime,cs.EndDateTime)=1 
	and (1=1) and (st.StateId is not null)'
...
set @SQLQuery = @SQLQuery  + 'GROUP by csdsend.Hours,csds.Hours,cm.HasMidNightCut,ws.IsSeparateInvoice,CAD.BuildingName ,
		...
		ws.WorkShiftName,ts.PayingHours,cc.Rate,cs.SchedulingId,ccr.Rate
...
SELECT
    cs.schedulingid,
			...
			((cs.EndDateTime)) as Enddate,
			cs.Description,
    CASE WHEN DATEDIFF(dd,cs.StartDateTime,cs.EndDateTime)=1   THEN Format(DATEADD(dd, DATEDIFF(dd, 0, cs.EndDateTime), 0),''hh:mm tt'')     ELSE format(cs.StartDateTime,''hh:mm tt'')
		END AS InTime,
		format(cs.EndDateTime,''hh:mm tt''),
...
set @SQLQuery = @SQLQuery  + 'GROUP by csdsend.Hours,csds.Hours,cm.HasMidNightCut,ws.IsSeparateInvoice,CAD.BuildingName ,
		ST.StateName,CAD.Zipcode,CAD.Phone1,ts.TimeShiftName,cm.ClientName,st.TaxPercent,st.StateId,
		ccr.IsTaxApplicable,cs.Description,cs.StartDateTime,cs.EndDateTime,ud.FirstName,ud.LastName,ct.TypeName,
		ws.WorkShiftName,ts.PayingHours,cc.Rate,cs.SchedulingId,ccr.Rate)t1
		...
		(select csd.SchedulingId,
			...
			sh.CountryId
			 ...
			 where cast(t1.StartDateTime as date ) in (select HolidayDate from Settings_Holidays) or cast(t1.EndDateTime as date ) in (select HolidayDate from Settings_Holidays)
		...
		t1.TotalHours-t1.RestTime as XHours,
		t2.HolidayPayValue,
		(select case when t2.HolidayPayValue !=  null or t2.HolidayPayValue > 0 then t2.HolidayPayValue *t1.rate else t1.rate end as r) as xRate,
		cast(isnull(t2.HoildayHours-t2.HolidayRestTime,0)as decimal(10,2))as HoildayHours ,		
		cast((isnull(t2.HoildayHours-t2.HolidayRestTime,0)*t1.rate)*t2.HolidayPayValue as decimal(10,2)) as  HoildayAmout ,
		...
		from 
			(SELECT cs.SchedulingId,cm.ClientName,@Currency as Currency,cs.StartDateTime,cs.EndDateTime,max(csd.Date) as detaildate,
			CAD.BuildingName ,ST.StateName,CAD.Zipcode,CAD.Phone1,
				@CurrencySymbol as CurrencySymbol,
...
'
					...
					set @SQLQuery=@SQLQuery + ' group by csdsend.Hours,csds.Hours,cm.HasMidNightCut,ws.IsSeparateInvoice,CAD.BuildingName ,
					ST.StateName,CAD.Zipcode,CAD.Phone1,ts.TimeShiftName,cm.ClientName,st.TaxPercent,st.StateId,ccr.IsTaxApplicable,
					cs.Description,cs.StartDateTime,cs.EndDateTime,ud.FirstName,ud.LastName,ct.TypeName,ws.WorkShiftName,ts.PayingHours,cc.Rate,cs.SchedulingId,ccr.Rate)t1
		left join
			(select csd.SchedulingId,
			sum(csd.Hours) as HoildayHours,
			(select case when sum(csd.Hours) >=8 
			...
			sh.CountryId
			 ...
			 group by csd.SchedulingId,sh.CountryId,shp.HolidayPayValue) t2
			 on t1.SchedulingId=t2.SchedulingId
			 where cast(t1.StartDateTime as date ) 
			 not in (select HolidayDate from Settings_Holidays) and cast(t1.EndDateTime as date ) not in (select HolidayDate from Settings_Holidays)
