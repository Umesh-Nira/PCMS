...
CREATE PROCEDURE [dbo].[SpClientIvoiceSearchWithDateSplit] --null,null,'01-apr-2019','30-apr-2019',null,null,0,4
(
	@CareTakerId int, 
	@CategoryId int, 
	@FromDate datetime,
...
select distinct * , 
		...
		CASE
				WHEN cast(t1.Startdate as date)in (select cast(HolidayDate as date) from Settings_Holidays) THEN cast(isnull(t2.HoildayHours-t2.HolidayRestTime,0)as decimal(10,2))
		ELSE 0
...
set @SQLQuery = @SQLQuery  + 'GROUP by csdsend.Hours,csds.Hours,cm.HasMidNightCut,ws.IsSeparateInvoice,CAD.BuildingName ,
		...
		ws.WorkShiftName,ts.PayingHours,cc.Rate,cs.SchedulingId,ccr.Rate,cm.InvoiceNumber,cm.InvoicePrefix
...
SELECT
    cs.schedulingid,cm.InvoiceNumber,cm.InvoicePrefix ,
			...
			cs.Description,		
			CASE 
			WHEN DATEDIFF(dd,cs.StartDateTime,cs.EndDateTime)=1   
		
			THEN Format(DATEADD(dd, DATEDIFF(dd, 0, cs.EndDateTime), 0),''hh:mm tt'')     
			ELSE format(cs.StartDateTime,''hh:mm tt'')
				END AS InTime,
		format(cs.EndDateTime,''hh:mm tt''),
			...
			ct.TypeName as ServiceTypeName,
			ws.WorkShiftName as WorkModeName,ws.IsSeparateInvoice,			
			(select case when cast((DATEDIFF(SECOND, cs.StartDateTime,cs.EndDateTime)*1.00/(60*60)) as decimal(10,2))>=8 or  csds.Hours >=8 
						then csds.Hours-(SELECT TOP 1 IntervalHours FROM [dbo].[Settings_IntervalHours]) 
						else csds.Hours end as starthrs) as StartHrs, 
            (select case when cast((DATEDIFF(SECOND, cs.StartDateTime,cs.EndDateTime)*1.00/(60*60)) as decimal(10,2))>=8 or csdsend.Hours >=8 
						then csdsend.Hours-(SELECT TOP 1 IntervalHours FROM [dbo].[Settings_IntervalHours]) 
						else csdsend.Hours end as endhrs) as EndHrs, 
			cast((DATEDIFF(SECOND, cs.StartDateTime,cs.EndDateTime)*1.00/(60*60)) as decimal(10,2)) as TotalHours,
			(select case when (DATEDIFF(SECOND, cs.StartDateTime,cs.EndDateTime)*1.00/(60*60)) >=8 
			then (SELECT TOP 1 IntervalHours FROM [dbo].[Settings_IntervalHours])
			else ''0''
