CREATE PROC [dbo].[SpSelectBookingHistoryDetail] --1087 --null
	(@BookingId INT) 
AS
BEGIN
	   ...
	   (cast((((t1.TotalHours-isnull(t2.HoildayHours,0))*isnull(t1.DisplayRate,0))  +  (isnull(t2.HoildayHours,0)*isnull(t1.DisplayRate,0))*isnull(t2.HolidayPayValue,0))  as decimal(10,2)) * (t1.taxx)) as TotalDisplayAmount
		...
		from 
			             (SELECT   b.BookingId,
								   ...
								   isnull(dbo.getCaretakerDisplayRate(cu.UserId,cs.ServiceId,b.ToDateTime),0.00) as DisplayRate,						                                 
								 ...
								 ( DateDiff(minute,b.FromDateTime,ToDateTime)  / 60.0) * isnull(dbo.getCaretakerDisplayRate(cu.UserId,cs.ServiceId,b.ToDateTime),0.00)  AS PayingAmount,
								 ( DateDiff(minute,b.FromDateTime,ToDateTime)  / 60.0) * isnull(dbo.getCaretakerPayingRate(cu.UserId,cs.ServiceId,b.ToDateTime),0.00) AS DisplayAmount,
						  ...
						  GROUP BY b.BookingId,b.BookingDateTime,cs.ServiceName,cu.FirstName,			 
								  ...
								  CN.Currency,ul.EmailId,ua.Phone1, cua.ZipCode,ppi.InvoiceNo,ppi.InvoicePath,s.TaxPercent,cs.ServiceId,cu.UserId)  t1
