...
CREATE VIEW [dbo].[vw_SchedulingFirstDate] AS
		SELECT
			cs.schedulingid,
			cm.InvoiceNumber,
			cm.InvoicePrefix,
			...
			isnull(dbo.getClientInvoiceRate(cs.ClientId,ct.TypeId,cs.StartDateTime),0.00) as  Rate,
...
GROUP by csdsend.Hours,csds.Hours,cm.HasMidNightCut,ws.IsSeparateInvoice,CAD.BuildingName ,
		...
		-- select * from vw_SchedulingFirstDate	where Startdate >= '2019-08-01'